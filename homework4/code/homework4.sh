...
for URL in $URLS; do
    ...
    ## Save the downloaded file with name $CURRENTINPUTFILE
    ...
    if (($? == 0)); then
        ...
        echo "$(file -b $CURRENTINPUTFILE)"
        file -b "$CURRENTINPUTFILE" | grep -q "PDF"
