/* 
   In this ROOT function we generate a distribution according to sin(x)
   between 0 and pi

   To run do:
   root 
   .L rootgenerate_v2.C 
   rootfuncgenerate(10000)
*/

// include C++ STL headers 
#include <iostream>

using namespace std;

// ROOT library obejcts
#include <TF1.h> // 1d function class
#include <TH1.h> // 1d histogram classes
#include <TStyle.h>  // style object
#include <TMath.h>   // math functions
#include <TCanvas.h> // canvas object
#include <fstream>

void rootfuncgenerate(Int_t nEvents, Int_t nTracks, Double_t v2); // ROOT method (a bit dangerous since we don't know exactly what happens!)


//________________________________________________________________________
void rootfuncgenerate(Int_t nEvents, Int_t nTracks, Double_t v2) 
{
  // open output file
  ofstream datafile("phi_dist.dat");

  cout << "Generating " << nEvents << " events" << endl << endl;

  // create histogram that we will fill with random values
  TH1D* hPhi = new TH1D("hPhi", "ROOT func f(phi) generated v2 distribution; phi; Counts", 
			100, 0, TMath::TwoPi());
  
  // Define the function we want to generate
  TF1* phiFunc = new TF1("phiFunc", Form("1+2*%f*cos(2*(x-0))", v2), 0, TMath::TwoPi());
  
  Double_t phi[nTracks]; //array to store phi angles

    // make a loop for the number of events
  for(Int_t n = 0; n < nEvents; n++) {
    datafile << "Event " << n << endl;
    datafile << "nTracks " << n << endl;
    
    for (Int_t nt = 0; nt < nTracks; nt++) {
      // Fill the array
      phi[nt] = phiFunc->GetRandom();
      // fill our event dist histogram
      hPhi->Fill(phi[nt]);
      datafile << nt << " : " << phi[nt] << endl;
    }
  }

  datafile.close();
  // Set ROOT drawing styles
  gStyle->SetOptStat(1111);
  gStyle->SetOptFit(1111);

  // create canvas for hPhi
  TCanvas* c1 = new TCanvas("c1", "Distribution of Event Plane", 900, 600);
  hPhi->SetMinimum(0);
  hPhi->Draw();
  
  // create 1d function that we will use to fit our generated data to ensure
  // that the generation works
  TF1* fitFunc = new TF1("fitFunc", "[0]*(1+2*[1]*cos(2*(x-0)))", 0, TMath::TwoPi());
  fitFunc->SetParameter(0, v2);
  fitFunc->SetLineColor(kRed);
  hPhi->Fit(fitFunc);
  
  // Save the canvas as a picture
  c1->SaveAs("v2_rootfunc.jpg");
}
